#!/usr/bin/env python3
"""
make-cover.py V15 定稿 — 統一封面生成器 v2.0（2026-08-10 用戶確認最終版）
...
FONT_DIR = "/home/hongk/.openclaw/workspace/jekyll-blog/assets/fonts"
FONT_BOLD = os.path.join(FONT_DIR, "NotoSansCJKsc-Black.otf")
...
def make_cover(bg, title_zh, title_en, out):
    ...
    canvas = Image.new("RGB", (tw, th), (13, 14, 18))
    canvas.paste(img, (0, max(0, (th - nh) // 2)))
    img = canvas
    w, h = img.size
    ...
    # ===== unwire 排版（正方形，置中；英文標題移到第三行/最底，2026-08-10 用戶要求） =====
    ...
    # 英文標題移到第三行（最底）
    if title_en:
        draw_center(draw, 700, title_en.upper(), font_en, GOLD, w, spacing=6, shadow=True)
        draw.line([(w // 2 - 100, 780), (w // 2 + 100, 780)], fill=GOLD, width=4)
